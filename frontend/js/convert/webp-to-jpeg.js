...
(function() {
  ...
  function initWebpToJpegView() {
    console.log('Initializing WebP to JPEG view');
    ...
    function handleFile(file) {
      // Validate file type - only accept WebP
      const allowedTypes = ['image/webp'];
      if (!allowedTypes.includes(file.type) && !file.name.match(/('..webp')$/i)) {
        alert('Please select a valid WebP image file (.webp)');
        return;
      }
    ...
    function processImageFile(file) {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          updateImageInfo('WebP', formatFileSize(file.size), `${img.width} × ${img.height}`);
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initWebpToJpegView);
  } else {
    initWebpToJpegView();
  }

  // Export for potential external use
  window.initWebpToJpegView = initWebpToJpegView;
