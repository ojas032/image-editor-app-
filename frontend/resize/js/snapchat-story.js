...
(function() {
  ...
  function initSnapchatsnapchatUstoryPage() {
  ...
  function setupYouTubeThumbnailFunctionality() {
    ...
    const SNAPCHAT_SNAPCHAT_STORY_PRESET = {
    ...
    function updateDimensionsDisplay() {
      ...
      if (dimensionsEl) {
        dimensionsEl.textContent = `Original: ${originalWidth} × ${originalHeight} → YouTube Thumbnail: ${SNAPCHAT_SNAPCHAT_STORY_PRESET.width} × ${SNAPCHAT_SNAPCHAT_STORY_PRESET.height}`;
      }
    }
    ...
    async function handleExport() {
      ...
      try {
        ...
        const requestData = {
          image: currentBase64,
          width: SNAPCHAT_SNAPCHAT_STORY_PRESET.width,
          height: SNAPCHAT_SNAPCHAT_STORY_PRESET.height,
          format: 'jpeg',
          quality: 95
        };
        ...
        downloadBtn.download = `snapchat-story-${Date.now()}.jpg`;

        // Update success dimensions
        const successDimensions = document.getElementById('resizeSuccessDimensions');
        if (successDimensions) {
          successDimensions.textContent = `Resized to: ${SNAPCHAT_SNAPCHAT_STORY_PRESET.width} × ${SNAPCHAT_SNAPCHAT_STORY_PRESET.height} pixels`;
        }
  ...
  window.initSnapchatsnapchatUstoryPage = initSnapchatsnapchatUstoryPage;
