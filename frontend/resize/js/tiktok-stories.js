...
(function() {
  ...
  function initTikToktiktokUstoriesPage() {
  ...
  function setupYouTubeThumbnailFunctionality() {
    ...
    const TIKTOK_TIKTOK_STORIES_PRESET = {
    ...
    function updateDimensionsDisplay() {
      ...
      if (dimensionsEl) {
        dimensionsEl.textContent = `Original: ${originalWidth} × ${originalHeight} → YouTube Thumbnail: ${TIKTOK_TIKTOK_STORIES_PRESET.width} × ${TIKTOK_TIKTOK_STORIES_PRESET.height}`;
      }
    }
    ...
    async function handleExport() {
      ...
      try {
        ...
        const requestData = {
          image: currentBase64,
          width: TIKTOK_TIKTOK_STORIES_PRESET.width,
          height: TIKTOK_TIKTOK_STORIES_PRESET.height,
          format: 'jpeg',
          quality: 95
        };
        ...
        downloadBtn.download = `tiktok-stories-${Date.now()}.jpg`;

        // Update success dimensions
        const successDimensions = document.getElementById('resizeSuccessDimensions');
        if (successDimensions) {
          successDimensions.textContent = `Resized to: ${TIKTOK_TIKTOK_STORIES_PRESET.width} × ${TIKTOK_TIKTOK_STORIES_PRESET.height} pixels`;
        }
  ...
  window.initTikToktiktokUstoriesPage = initTikToktiktokUstoriesPage;
