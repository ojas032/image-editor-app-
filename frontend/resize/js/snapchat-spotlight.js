...
(function() {
  ...
  function initSnapchatsnapchatUspotlightPage() {
  ...
  function setupYouTubeThumbnailFunctionality() {
    ...
    const SNAPCHAT_SNAPCHAT_SPOTLIGHT_PRESET = {
      name: 'Spotlight (1080 × 1920)',
      width: 1080,
      height: 1920
    };
    ...
    function updateDimensionsDisplay() {
      ...
      if (dimensionsEl) {
        dimensionsEl.textContent = `Original: ${originalWidth} × ${originalHeight} → YouTube Thumbnail: ${SNAPCHAT_SNAPCHAT_SPOTLIGHT_PRESET.width} × ${SNAPCHAT_SNAPCHAT_SPOTLIGHT_PRESET.height}`;
      }
    }
    ...
    async function handleExport() {
      ...
      try {
        ...
        const requestData = {
          image: currentBase64,
          width: SNAPCHAT_SNAPCHAT_SPOTLIGHT_PRESET.width,
          height: SNAPCHAT_SNAPCHAT_SPOTLIGHT_PRESET.height,
          format: 'jpeg',
          quality: 95
        };
        ...
        downloadBtn.download = `snapchat-spotlight-${Date.now()}.jpg`;

        // Update success dimensions
        const successDimensions = document.getElementById('resizeSuccessDimensions');
        if (successDimensions) {
          successDimensions.textContent = `Resized to: ${SNAPCHAT_SNAPCHAT_SPOTLIGHT_PRESET.width} × ${SNAPCHAT_SNAPCHAT_SPOTLIGHT_PRESET.height} pixels`;
        }
  ...
  window.initSnapchatsnapchatUspotlightPage = initSnapchatsnapchatUspotlightPage;
