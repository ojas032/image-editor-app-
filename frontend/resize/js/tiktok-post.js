...
(function() {
  ...
  function initTikToktiktokUpostPage() {
  ...
  function setupYouTubeThumbnailFunctionality() {
    ...
    const TIKTOK_TIKTOK_POST_PRESET = {
      name: 'Post (1080 × 1920)',
      width: 1080,
      height: 1920
    };
    ...
    function updateDimensionsDisplay() {
      ...
      if (dimensionsEl) {
        dimensionsEl.textContent = `Original: ${originalWidth} × ${originalHeight} → YouTube Thumbnail: ${TIKTOK_TIKTOK_POST_PRESET.width} × ${TIKTOK_TIKTOK_POST_PRESET.height}`;
      }
    }
    ...
    async function handleExport() {
      ...
      try {
        ...
        const requestData = {
          image: currentBase64,
          width: TIKTOK_TIKTOK_POST_PRESET.width,
          height: TIKTOK_TIKTOK_POST_PRESET.height,
          format: 'jpeg',
          quality: 95
        };
        ...
        downloadBtn.download = `tiktok-post-${Date.now()}.jpg`;

        // Update success dimensions
        const successDimensions = document.getElementById('resizeSuccessDimensions');
        if (successDimensions) {
          successDimensions.textContent = `Resized to: ${TIKTOK_TIKTOK_POST_PRESET.width} × ${TIKTOK_TIKTOK_POST_PRESET.height} pixels`;
        }
  ...
  window.initTikToktiktokUpostPage = initTikToktiktokUpostPage;
