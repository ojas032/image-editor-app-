...
(function() {
  ...
  function initTwittertwitterUstreamUlinkPage() {
  ...
  function setupYouTubeThumbnailFunctionality() {
    ...
    const TWITTER_TWITTER_STREAM_LINK_PRESET = {
      name: 'Stream Link (1200 × 675)',
      width: 1200,
      height: 675
    };
    ...
    function updateDimensionsDisplay() {
      ...
      if (dimensionsEl) {
        dimensionsEl.textContent = `Original: ${originalWidth} × ${originalHeight} → YouTube Thumbnail: ${TWITTER_TWITTER_STREAM_LINK_PRESET.width} × ${TWITTER_TWITTER_STREAM_LINK_PRESET.height}`;
      }
    }
    ...
    async function handleExport() {
      ...
      try {
        ...
        const requestData = {
          image: currentBase64,
          width: TWITTER_TWITTER_STREAM_LINK_PRESET.width,
          height: TWITTER_TWITTER_STREAM_LINK_PRESET.height,
          format: 'jpeg',
          quality: 95
        };
        ...
        downloadBtn.download = `twitter-stream-link-${Date.now()}.jpg`;

        // Update success dimensions
        const successDimensions = document.getElementById('resizeSuccessDimensions');
        if (successDimensions) {
          successDimensions.textContent = `Resized to: ${TWITTER_TWITTER_STREAM_LINK_PRESET.width} × ${TWITTER_TWITTER_STREAM_LINK_PRESET.height} pixels`;
        }
  ...
  window.initTwittertwitterUstreamUlinkPage = initTwittertwitterUstreamUlinkPage;
