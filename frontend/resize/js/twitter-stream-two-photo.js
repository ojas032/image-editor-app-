...
(function() {
  ...
  function initTwittertwitterUstreamUtwoUphotoPage() {
  ...
  function setupYouTubeThumbnailFunctionality() {
    ...
    const TWITTER_TWITTER_STREAM_TWO_PHOTO_PRESET = {
      name: 'Stream Two Photo (1200 × 675)',
      width: 1200,
      height: 675
    };
    ...
    function updateDimensionsDisplay() {
      ...
      if (dimensionsEl) {
        dimensionsEl.textContent = `Original: ${originalWidth} × ${originalHeight} → YouTube Thumbnail: ${TWITTER_TWITTER_STREAM_TWO_PHOTO_PRESET.width} × ${TWITTER_TWITTER_STREAM_TWO_PHOTO_PRESET.height}`;
      }
    }
    ...
    async function handleExport() {
      ...
      try {
        ...
        const requestData = {
          image: currentBase64,
          width: TWITTER_TWITTER_STREAM_TWO_PHOTO_PRESET.width,
          height: TWITTER_TWITTER_STREAM_TWO_PHOTO_PRESET.height,
          format: 'jpeg',
          quality: 95
        };
        ...
        downloadBtn.download = `twitter-stream-two-photo-${Date.now()}.jpg`;

        // Update success dimensions
        const successDimensions = document.getElementById('resizeSuccessDimensions');
        if (successDimensions) {
          successDimensions.textContent = `Resized to: ${TWITTER_TWITTER_STREAM_TWO_PHOTO_PRESET.width} × ${TWITTER_TWITTER_STREAM_TWO_PHOTO_PRESET.height} pixels`;
        }
  ...
  window.initTwittertwitterUstreamUtwoUphotoPage = initTwittertwitterUstreamUtwoUphotoPage;
