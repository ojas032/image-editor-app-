...
(function() {
  ...
  function initTwittertwitterUheaderUphotoPage() {
  ...
  function setupYouTubeThumbnailFunctionality() {
    ...
    const TWITTER_TWITTER_HEADER_PHOTO_PRESET = {
      name: 'Header Photo (1500 × 500)',
      width: 1500,
      height: 500
    };
    ...
    function updateDimensionsDisplay() {
      ...
      if (dimensionsEl) {
        dimensionsEl.textContent = `Original: ${originalWidth} × ${originalHeight} → YouTube Thumbnail: ${TWITTER_TWITTER_HEADER_PHOTO_PRESET.width} × ${TWITTER_TWITTER_HEADER_PHOTO_PRESET.height}`;
      }
    }
    ...
    async function handleExport() {
      ...
      try {
        ...
        const requestData = {
          image: currentBase64,
          width: TWITTER_TWITTER_HEADER_PHOTO_PRESET.width,
          height: TWITTER_TWITTER_HEADER_PHOTO_PRESET.height,
          format: 'jpeg',
          quality: 95
        };
        ...
        downloadBtn.download = `twitter-header-photo-${Date.now()}.jpg`;

        // Update success dimensions
        const successDimensions = document.getElementById('resizeSuccessDimensions');
        if (successDimensions) {
          successDimensions.textContent = `Resized to: ${TWITTER_TWITTER_HEADER_PHOTO_PRESET.width} × ${TWITTER_TWITTER_HEADER_PHOTO_PRESET.height} pixels`;
        }
  ...
  window.initTwittertwitterUheaderUphotoPage = initTwittertwitterUheaderUphotoPage;
