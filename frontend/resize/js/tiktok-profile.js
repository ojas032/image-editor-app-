...
(function() {
  ...
  function initTikToktiktokUprofilePage() {
  ...
  function setupYouTubeThumbnailFunctionality() {
    ...
    const TIKTOK_TIKTOK_PROFILE_PRESET = {
      name: 'Profile Picture (200 × 200)',
      width: 200,
      height: 200
    };
    ...
    function updateDimensionsDisplay() {
      ...
      if (dimensionsEl) {
        dimensionsEl.textContent = `Original: ${originalWidth} × ${originalHeight} → YouTube Thumbnail: ${TIKTOK_TIKTOK_PROFILE_PRESET.width} × ${TIKTOK_TIKTOK_PROFILE_PRESET.height}`;
      }
    }
    ...
    async function handleExport() {
      ...
      try {
        ...
        const requestData = {
          image: currentBase64,
          width: TIKTOK_TIKTOK_PROFILE_PRESET.width,
          height: TIKTOK_TIKTOK_PROFILE_PRESET.height,
          format: 'jpeg',
          quality: 95
        };
        ...
        downloadBtn.download = `tiktok-profile-${Date.now()}.jpg`;

        // Update success dimensions
        const successDimensions = document.getElementById('resizeSuccessDimensions');
        if (successDimensions) {
          successDimensions.textContent = `Resized to: ${TIKTOK_TIKTOK_PROFILE_PRESET.width} × ${TIKTOK_TIKTOK_PROFILE_PRESET.height} pixels`;
        }
  ...
  window.initTikToktiktokUprofilePage = initTikToktiktokUprofilePage;
