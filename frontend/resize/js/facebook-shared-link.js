...
(function() {
  ...
  function initFacebookfacebookUsharedUlinkPage() {
  ...
  function setupYouTubeThumbnailFunctionality() {
    ...
    const FACEBOOK_FACEBOOK_SHARED_LINK_PRESET = {
      name: 'Shared Link (1200 × 630)',
      width: 1200,
      height: 630
    };
    ...
    function updateDimensionsDisplay() {
      ...
      if (dimensionsEl) {
        dimensionsEl.textContent = `Original: ${originalWidth} × ${originalHeight} → YouTube Thumbnail: ${FACEBOOK_FACEBOOK_SHARED_LINK_PRESET.width} × ${FACEBOOK_FACEBOOK_SHARED_LINK_PRESET.height}`;
      }
    }
    ...
    async function handleExport() {
      ...
      try {
        ...
        const requestData = {
          image: currentBase64,
          width: FACEBOOK_FACEBOOK_SHARED_LINK_PRESET.width,
          height: FACEBOOK_FACEBOOK_SHARED_LINK_PRESET.height,
          format: 'jpeg',
          quality: 95
        };
        ...
        downloadBtn.download = `facebook-shared-link-${Date.now()}.jpg`;

        // Update success dimensions
        const successDimensions = document.getElementById('resizeSuccessDimensions');
        if (successDimensions) {
          successDimensions.textContent = `Resized to: ${FACEBOOK_FACEBOOK_SHARED_LINK_PRESET.width} × ${FACEBOOK_FACEBOOK_SHARED_LINK_PRESET.height} pixels`;
        }
  ...
  window.initFacebookfacebookUsharedUlinkPage = initFacebookfacebookUsharedUlinkPage;
